...
\setbeamercolor{frametitle}{fg=orange, bg=brown!25}
...
\usetikzlibrary{arrows.meta, shadows, shapes, arrows, positioning}
...
%     \node[above left=1.3cm and 2.0cm of current page.south east] {\includegraphics[scale=0.5]{text/UiO_Segl_pms485.eps}};
% \end{tikzpicture}
% \end{frame}
...
%         \item Interaction: $f\left(\boldsymbol{r}\right) = \sum\limits_{i<j}
%             \frac{1}{|\boldsymbol{r}_i - \boldsymbol{r}_j|}$
    ...
    \end{itemize}
    \vspace{-0.25cm}
    \onslide<4->{\hspace{1.6cm}$V(\blds{r}) = \frac{1}{2} m\omega^2r^2 \hspace{1.6cm}
...
\begin{frame}[fragile]{Methods: Hartree-Fock}
    \begin{itemize}[<+->]
        \item Single Slater determinant wavefunction
        \item Constrained minimization
            \begin{itemize}
                \item Spin orthogonality: $\braket{\psi_i}{\psi_j} =
                    \delta_{ij}$
                \item Fock-operator: $\mathcal{F} \equiv \mathcal{H}_0 + \mathcal{J} - \mathcal{K}$
                    \begin{itemize}[<4->]
    ...
    \begin{itemize}[<+->]
        ...
        \item Rewrite expectation value:
            ...
            \begin{itemize}
                ...
                \item \color<4>{red}{$P(\mb{R}) \equiv \frac{\abs{\Psi}^2}{\Braket{\Psi|\Psi}}$}
            \end{itemize}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            \node (hupdate) [process, above of=no, xshift=-2.0cm, yshift=0.5cm] {
                Use $\nabla f(\blds{x}_n)$ and $\nabla f(\blds{x}_{n-1})$ in an \\
...
{\setbeamercolor{palette primary}{fg=black, bg=white}
\begin{frame}[standout]{Minimization: Simulated Annealing}
    \animategraphics[controls, scale=0.5]{10}{text/sianData/fig-}{1}{99}
\end{frame}}
    ...
    \begin{itemize}[<+->]
        ...
        \item Solution in Cartesian\only<4->{\footfullcite{HelgakerMolElcTheory}} \\ \vspace{0.2cm} \hspace{1cm}
            \begin{minipage}[H]{0.5\textwidth}
                ...
                $\Braket{g_i(\blds{r})|x^l_d|g_j(\blds{r})}$ \vsp \\
...
\begin{frame}[fragile]{Wavefunction: Slater-Jastrow}
    \begin{itemize}[<+->]
        \item Slater determinant: $\psi = \det(\blds{\Phi})J $
        ...
        \begin{tikzpicture}[
            >={Latex[width=2mm,length=2mm]},
                ...
                basecode/.style = {rectangle, rounded corners, draw=black,
                            ...
                            centered, font=\sffamily, align=left},
                activityStarts/.style = {base, fill=blue!30, drop shadow},
                activityStartscode/.style = {basecode, fill=blue!30, drop shadow},
                startstop/.style = {base, fill=red!25, drop shadow},
                startstopcode/.style = {basecode, fill=red!25, drop shadow},
                activityRuns/.style = {base, fill=green!25, drop shadow},
                activityRunscode/.style = {basecode, fill=green!25, drop shadow},
                process/.style = {base, fill=white!15, font=\sffamily, drop shadow},
                processcode/.style = {basecode, fill=white!15, font=\sffamily, drop shadow},
            scale=0.8, 
            node distance=1.25cm, 
            every node/.style={fill=white, font=\sffamily, scale=0.6},
            align=center]
            ...
            \node (Cartesian) [processcode, below of=GaussianQuadrature] {
                \hltexttt{Cartesian}
            };
            \node (Hermite) [processcode, below of=Cartesian] {
                \hltexttt{Hermite}
            };
            \node (HartreeFockSolver) [processcode, below of=Hexpander, yshift=-3.0cm] {
                \hltexttt{HartreeFockSolver}
            };
            \node (GaussianIntegrals) [activityRunscode, above right of=HartreeFockSolver, xshift=4cm] {
                \hltexttt{GaussianIntegrals}
            };
            \node (DoubleWell) [activityRunscode, below right of=HartreeFockSolver, xshift=4cm] {
                \hltexttt{DoubleWell}
            };
            \node (DWC) [processcode, below of=Hermite, yshift=-2cm] {
                \hltexttt{DWC}
            };
...
\begin{frame}[fragile]{Implementation: Variational Monte-Carlo}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            scale=0.8, 
            node distance=1.25cm, 
            every node/.style={fill=white, font=\sffamily, scale=0.51},
            align=center]
            \node (HarmonicOscillator) [startstopcode] {
                \hltexttt{HarmonicOscillator}
            };
            \node (HartreeFock) [startstopcode, below of=HarmonicOscillator] {
                \hltexttt{HartreeFock}
            };
            \node (VariationalHartreeFock) [startstopcode, below of=HartreeFock] {
                \hltexttt{VariationalHartreeFock}
            };
            \node (HartreeFockDoubleWell) [startstopcode, below of=VariationalHartreeFock] {
                \hltexttt{HartreeFockDoubleWell}
            };
            \node (VariationalHartreeFockDoubleWell) [startstopcode, below of=HartreeFockDoubleWell] {
                \hltexttt{VariationalHartreeFockDoubleWell}
            };
            \node (PadeJastrow) [activityRunscode, below of=VariationalHartreeFockDoubleWell, xshift=-2cm, yshift=-1cm] {
                \hltexttt{PadeJastrow}
            };
            \node (RBMJastrow) [activityRunscode, below of=PadeJastrow, yshift=-2cm] {
                \hltexttt{RBMJastrow}
            };
            \node (PadeNQS) [activityRunscode, below right of=PadeJastrow, xshift=2.0cm] {
                \hltexttt{PadeNQS}
            };
            \node (ExpNQS) [activityRunscode, below of=PadeNQS] {
                \hltexttt{ExpNQS}
            };
            ...
            \node (ReSampler) [processcode, above of=VMC, yshift=1.5cm] {
                \hltexttt{ReSampler}
            };
            \draw[->] (ReSampler) to [out=-90, in=90] (VMC);
    ...
    \begin{itemize}[<2->]
        ...
        \item<3-> Wavefunction class generated with Python
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Results: Benchmark}
    \vspace{-0.25cm}
    \twofigure{text/figs/N2_D2.pdf}{text/figs/N6_D2.pdf}
    \vspace{-0.5cm}
    \twofigure{text/figs/N12_D2.pdf}{text/figs/N20_D2.pdf}
\end{frame}

\begin{frame}[fragile]{Results: Benchmark}
    \vspace{-0.25cm}
    \twofigure{text/figs/N30_D2.pdf}{text/figs/N42_D2.pdf}
    \vspace{-0.5cm}
    \onefigure{text/figs/N56_D2.pdf}
\end{frame}
...
\begin{frame}[fragile]{Results: Double-Well Hartree-Fock}
    \hspace{2.6cm}2D \hspace{4.8cm}3D
    \twofigure{text/figs/D2_DW.pdf}{text/figs/D3_DW.pdf}
\end{frame}
...
\begin{frame}[fragile]{Results: NQS-Jastrow Harmonic Oscillator}
    \vspace{-0.4cm}
    \twofigure{{text/figs/w1.0_D2_N2_Energies_fig}.pdf}{{text/figs/w1.0_D2_N6_Energies_fig}.pdf}
    \vspace{-0.6cm}
    \twofigure{{text/figs/w1.0_D3_N2_Energies_fig}.pdf}{{text/figs/w1.0_D3_N8_Energies_fig}.pdf}
\end{frame}

\begin{frame}[fragile]{Results: NQS-Jastrow Double-Well}
    \vspace{-0.4cm}
    \twofigure{{text/figs/w1.0_D2_N2_Energies_DW_fig}.pdf}{{text/figs/w1.0_D2_N4_Energies_DW_fig}.pdf}
    \vspace{-0.6cm}
    \twofigure{{text/figs/w1.0_D3_N2_Energies_DW_fig}.pdf}{{text/figs/w1.0_D3_N4_Energies_DW_fig}.pdf}
\end{frame}
...
\begin{frame}[fragile]{Summary}
    \begin{itemize}
        \item Schrödinger equation: $\mathcal{H}\ket{\psi} = E\ket{\psi}$,
            ...
            f\left(\blds{r}\right) + V\left(\blds{R},\blds{r}\right)$
        \item Interaction: $f\left(\blds{r}\right) = \sum\limits_{i<j} \frac{1}{|r_i - r_j|}$
        \item Confinement: Harmonic Oscillator, Double-Well
    \end{itemize}
    \vspace{-0.25cm}
    \hspace{1.6cm}$V(\blds{r}) = \frac{1}{2} m\omega^2r^2 \hspace{1.6cm}
    V(\blds{r}) = \frac{1}{2}m\omega^2\left(r^2 - \delta R\abs{x} + R^2\right)$
    \vspace{-0.40cm}\twofigure{text/figs/HO2Dplot.pdf}{text/figs/DW2Dplot.pdf}
\end{frame}
