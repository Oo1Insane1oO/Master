...
\chapter{Numerical Optimization \label{chapter:5}}
    ...
    numerical optimization methods can be applied. As such, the field in
    numerical optimization has not been viewed more useful than now, especially
    with the computing power at hand and machine learning having become
    somewhat of a popular hit. 
    ...
    from \txtit{numerical optimization}. The methods used in this thesis was
    the \txtit{Conjugate Gradient method}, a version of the \txtit{Adaptive
    Stochastic Gradient Descent}, the well known \txtit{BFGS} method and a more
    resent \txtit{Stochastic-Adaptive-BFGS}. The explaining of the approaches
    for numerical optimization is only made briefly. For a better mathematical
...
\section{The Optimization Problem \label{sec:the_optimization_problem}}
    ...
    means we seek a point $\blds{\alpha}_m$ in real space were the variation of
    the value of $f$ is zero. In reality the condition in \Arf{eq:mincondition}
    is only approximate, that is we terminate the search for a minimum if we
...
\section{Gradient Descent\label{sec:gradient_descent}}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            \node (start) [activityStarts] {
                Initialize $\blds{\alpha}_0$ and find
                $\nabla_{\alpha}f({\blds{\alpha}_0})$ \\
                Set $\gamma$ and $n=1$.
            };
            \node (update) [activityRuns, below of=start] {
                Find $\blds{\alpha}_{n} = \blds{\alpha}_{n-1} - \gamma
                \nabla_{\alpha} f$
            };
            \draw[->] (start) -- (update);
            ...
            \node (end) [activityStarts, below of=yes] {
                Output $\blds{\alpha}_m=\blds{\alpha}_n$
            };
            \draw[->] (yes) -- (end);
        \end{tikzpicture}
    ...
    optimal and the step-size $\gamma$ is a constant which can give allot of
    oscillations around minimum as the algorithm might get close to the minimum
    and then \txtit{over-shoot} and go past the minimum point, turn
    around(because the sign changes) and over-shoot again and then keep going.
    Many(seriously many) methods have been devised to account for these
