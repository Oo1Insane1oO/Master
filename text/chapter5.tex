...
Methods\label{sec:newtons-method_and_quasi-newton_methods}}
    ...
    step-length multiplied to the Hessian part in order to induce conditions >>
    REF THIS << which ensure some stability of the method. \\
    ...
    formula} for the inverse. This basically means that the need calculation of
    the inverse matrix is completely removed. \\

    With the mention expression we can devise an algorithm similar to Newton's
...
\section{BFGS\label{sec:BFGS}}
    ...
    here >> REF BFGS <<. The conditions enforced is
        \begin{itemize}
    ...
    \nabla f(\blds{x}_n)$
        \begin{equation}
            ...
            \label{eq:BFGSdef}
        \end{equation}
    With the Sherman-Morrison formula the inverse is updated with
        \begin{equation}
