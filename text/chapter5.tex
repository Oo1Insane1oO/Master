...
\subsection{Parallelization of Two-Body Matrix}
    ...
    Ns))$ in the one-dimensional array. The symmetry 
        \begin{equation}
            (p,q,r,s)=(q,p,r,s)
        \end{equation}
...
\subsection{Slater Optimizations\label{susec:slateropt}}
    ...
    needing two inverses!? Fret not, the Sherman-Morrison
...
\section{Verification\label{sec:verification}}
    ...
    harmonic oscillator potential. The test are set-up with the
    \pp{UnitTest}\cite{UnitTest++} and one only needs to pass a flag $TESTS=ON$
    at compile time to enable them.
...
\subsection{Variational Monte-Carlo}
    ...
    variational parameters} as the wavefunction ansatz
        \begin{equation}
