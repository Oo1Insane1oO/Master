...
\chapter{Numerical Optimization \label{chapter:5}}
    ...
    the VMC case) the wavefunction using methods from a popular field in
    mathematics called \txtit{numerical optimization}. The methods used in this
    thesis was the \txtit{Conjugate Gradient method}, a version of the
    \txtit{Adaptive Stochastic Gradient Descent}, the well known \txtit{BFGS}
    method and a more resent \txtit{Stochastic-Adaptive-BFGS}. The explaining
    of the approaches for numerical optimization is only explained briefly. For
    a better mathematical explanation see >> REF THESE <<
