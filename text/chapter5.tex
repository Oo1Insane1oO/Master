...
\chapter{Implementation \label{chapter:5}}
    ...
    challenge and most like\footnote{Read definitely.} impossible, but with the
    modern computer at our disposal they can be tackled numerically. However in
    order to get the data from the numerical simulations the actual
    implementation, the raw code, has to be made, tested, verified and run. \\
    ...
    for the first few levels
        \begin{figure}[H]
            ...
            \begin{subfigure}[b!]{0.45\textwidth}
                ...
                \end{tikzpicture}
                \caption{Two-dimensional case with $(n_x,n_y)$}
                \label{subfig:2Dspin}
            \end{subfigure}
            \begin{subfigure}[b!]{0.45\textwidth}
                \centering
                ...
                \end{tikzpicture}
                \caption{Three-dimensional case with $(n_x,n_y,n_z)$}
                \label{subfig:3Dspin}
            \end{subfigure}
...
\subsection{Parallelization of Two-Body Matrix}
    ...
    matrix then created with elements
        \begin{equation}
            \text{pqMap}_{pq} = (p,q)
        \end{equation}
    ...
    size of each chunk is defined by \Arf{eq:divideeven}.  The algorithm is as
    follows
        \begin{algorithm}[H]
    ...
    elements received.  Each sub-chunk is then sent to root process and
    concatenated to a large one-dimensional array and the actual two-body
    matrix of size $N^4$ is assembled and antisymmetrized. The Hartree-Fock
    algorithm is then run only on one process.
    ...
    \Arf{fig:minimizeProc}.
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[
                ...
                \node (convergence) [process, below left of=updater, xshift=-0.5cm, yshift=-0.3cm] {
                    Check for \\
                    convergence.
                };
                \node (exhaustion) [process, below right of=updater, xshift=1.1cm, yshift=-0.3cm] {
                    Switch to input \\
                    method if exhausted
                };
                ...
                \draw[->] (loop) -- (updater);
                \node (callFunc) [startstop, below of=updater, yshift=-2cm] {
