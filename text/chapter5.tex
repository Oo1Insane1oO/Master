...
\chapter{Implementation \label{chapter:5}}
    ...
    challenge and most likely impossible, but with the modern computer at our
    disposal they can be tackled numerically.  However in order to get the data
    from the numerical simulations the actual implementation, the code, has to
    be written, tested, verified and run. \\
...
\section{Hartree-Fock}
    Only the restricted case is implemented and is present as
...
\subsection{Parallelization of Two-Body Matrix}
    ...
    $q$ value. \\
    The rows are then distributed evenly among $P$ processes according to this
    \Arf{eq:divideeven},
        \begin{equation}
...
\section{Variational Monte Carlo}
    ...
    The template is built in such a way that one only needs to fill in specific
    analytic expressions for the gradient, Laplacian and gradient for the
    variational parameters. The latter part is optional. \\ 
    ...
    array) is specific to each system. 
...
\subsection{Thermalisation}
    ...
    unreasonably high variance. To avoid this we \txtit{thermalize} the system
...
\subsection{Calculation of Hermite Polynomials}
...
\section{Verification\label{sec:verification}}
    ...
    in \Arf{tab:HOHF, tab:HOHF3D, tab:HObench2D, tab:HF2DVMCHO,
    tab:HFVAR2DVMCHO, tab:HObench3D, tab:HF3DVMCHO, tab:HFVAR3DVMCHO}.
...
\subsection{Hartree-Fock}
    ...
    perturbation of the single-well system. The results are given in
    \Arf{tab:HODW2D, tab:HODW3D, tab:HF2DVMCDW, tab:HF3DVMCDW,
    tab:HFVAR2DVMCDW, tab:HFVAR3DVMCDW}.
