    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
                >={Latex[width=2mm,length=2mm]},
                    ...
                    process/.style = {base, fill=orange!15, font=\sffamily},
                scale=0.8, 
                node distance=1.5cm, 
            ...
            \node (initIntegral) [activityStarts, below of=main] {
                ...
                \\ Iterate};
            \node (integralcode) [process, right of=initIntegral, xshift=6cm] {
                \lstinline[language=C++, style=ccstyle]{Integral$^*$ HFS = new Integral(D, L, N);} \\
                \lstinline[language=C++, style=ccstyle]{string message = HFS->initializeParameters(...);}};
            ...
            \node (assemble) [startstop, right of=iteratecode, xshift=6cm] {
            ...
            \node (hflgoexplfock) [startstop, below of=hfloopstart] {
                Set Fock-Matrix with \Arf{eq:FockRestrictedDef}
            };
            \node (hfleig) [startstop, below of=hflgoexplfock] {
                Solve eigenvalue \Arf{eq:eigvalrestr} 
            };
            \node (hflgoexpldens) [startstop, below of=hfleig] {
                Set density matrix with \Arf{eq:densitymatrixdef}
            };
            \node (hflconvtest) [startstop, below of=hflgoexpldens] {
                Break if converged
            };
            ...
            \draw[->] (hfloopstart) -- (hflgoexplfock);
            \draw[->] (hflgoexplfock) -- (hfleig);
            \draw[->] (hfleig) -- (hflgoexpldens);
            \draw[->] (hflgoexpldens) -- (hflconvtest);
            \draw[->] (hfalgo) -- (finish);
        \end{tikzpicture}
