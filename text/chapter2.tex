...
\chapter{Many-Body Quantum Theory\label{chapter:2}}
    ...
    \txtit{Dirac-notation} used throughout the entire chapter.
...
\subsection{Quantum Dot System}
    ...
    With again thesis of Merlot and Ming in mind with
    article by Simen Kvaal\cite{kvaaldots} one model for the
    \txtit{confinement} potential is the Harmonic Oscillator potential
        \begin{equation}
            V(\blds{R},\blds{r}) = \frac{1}{2}mr^2.
        \end{equation}
    A third approach is to use a \txtit{double-dot} which is basically just a
    shifted harmonic oscillator\cite{ddotnuc,yangthesis}. For simplicity we
    ...
    Young, Muller and Carroll\cite{nonIsoGauss}
        \begin{equation}
            V(\blds{R},\blds{r}) = \frac{1}{2}m\omega^2\left(r^2 - 2R\abs{x} +
            R^2\right)
        \end{equation}
    ...
    Here is also a figure describing the algorithm.
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[
                ...
                \node (Fockmix) [activityRuns, below of=Fock, yshift=-0.65cm] {
