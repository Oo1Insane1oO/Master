...
\chapter{Many-Body Quantum Theory\label{chapter:2}}
    ...
    \txtit{many-body quantum theory} and further deepen into the Hartree-Fock
...
\subsubsection{Confinement Potential}
    ...
    Taking our inspiration from the theses of Merlot and Ming and an article by
    Simen Kvaal\cite{kvaaldots}, one model for the \txtit{confinement}
    potential is the harmonic oscillator potential, a so-called
    \txtit{parabolic dot}.(see \Arf{subfig:HO1D, subfig:HO2D})
        \begin{equation}
    ...
    A second approach is to use a \txtit{double-dot} which is basically just
...
\section{Slater Determinant and Permanent}
    ...
    \txtit{identical and indistinguishable particles}. Since the particles are
    ...
    or antisymmetric with the symmetric part being the \txtit{bosonic state}
    and antisymmetric being the \txtit{fermionic state}. The connection between
    antisymmetric states and fermions is called the \txtit{Pauli exclusion
    principle}.
...
\section{Hartree-Fock Theory\label{sec:HFtheory}}
    ...
    many methods that try to fix this problem, but the \txtit{Variational Monte
    Carlo} (or VMC) is the method for deeper explorations in this Thesis, see
    \Arf{sec:QMC} for more details.
...
\section{Unrestricted Hartree-Fock and Poople-Nesbet-Equations\label{sec:UHFPNE}}
    ...
    in this case called the \txtit{Poople-Nesbet equations}. The derivations
    are exactly the same as for the restricted case, but without the
    spin-pairing in \Arf{eq:spinpairingHF}. We get two equations, one for the
...
\section{Quantum Monte Carlo\label{sec:QMC}}
    ...
    \subsection{The Variational Principle and Expectation Value of Energy}
        ...
        \Arf{susec:diffTHpdf} for a more detailed reasoning)
            \begin{equation}
        ...
        \Arf{chapter:5}). We essentially minimize the expectation value of the
        energy, see \Arf{susec:MHAlg}.
    ...
    \subsection{Importance Sampling\label{susec:impSamp}}
        ...
        Which finally gives
            \begin{equation}
                ...
                \label{eq:qForceDef}
            \end{equation}
        This is the so-called \txtit{quantum force} responsible for pushing the
    ...
    \subsection{The Trial Wavefunction: One-Body}
        ...
        introduced in \Arf{susec:TWFJastrow}. In this case the
    ...
    \subsection{The Trial Wavefunction: Splitting the Slater Determinant}
        ...
        The procedure is simply to arrange the basis functions in the following
        Slater determinant
            \begin{equation}
    ...
    \subsection{The Trial Wavefunction: Jastrow Factor\label{susec:TWFJastrow}}
        ...
        Slater determinant and a correlation part known as a \txtit{Jastrow
        factor}. The Jastrow can have many forms, but is build to exert some
        ...
        \txtit{Pad\'e-function} and a more recent one built with a specific
...
\section{Statistics and Blocking}
    ...
    \txtit{blocking}. The details are explained with article \cite{blocking} by
    ...
    is reached. \\
    However the thesis of Marius Jonsson >>REF<< the block-size is estimated with a
    neat algorithm which we also use.

\section{Density Matrices\label{sec:densityMat}}
    ...
    the so-called \txtit{one-body density}. From quantum mechanics the one-body
    ...
    need to divide away the dimensional volume element of each bin. The element
