...
\section{Quantum Monte Carlo\label{sec:QMC}}
    ...
    \subsection{The Trial Wavefunction: Splitting the Slater Determinant}
        ...
        Hartree-Fock method). For the splitting with a \txtit{spin-independent}
    ...
    \subsection{The Trial Wavefunction: Jastrow\label{susec:TWFJastrow}}
        ...
        Jastrow can have many form, however a popular form is the
        ...
        meaning the trial wavefunction is
            \begin{equation}
                \psiT = \det(\blds{\Phi})J
            \end{equation}
        where the $\beta_l$'s are variational parameters and $a^{(l)}_{ij}$ is
        a spin-dependant parameters which ensures a \txtit{cusp condition}.
