...
\chapter{Basis Functions\label{chapter:3}}
    ...
    the Schrödinger equation and the space at hand is the \txtit{Hilbert
...
\section{Hermite Functions}
    ...
    \txtit{quantum harmonic oscillator system} \cite{GriffQuan} with the scaling
...
\section{Gaussian Type Orbitals}
    \txtit{Gaussian Type Orbitals} or GTO's are functions of the form
    ...
    literature referred to as \txtit{primitive Gaussians} and they alone make
...
\section{Integral Elements}
    ...
    Before we calculate the integrals, let us first express the
...
\subsection{Overlap Distribution}
    ...
    have\footnote{The indices $i$ and $j$ are now in 1 dimension!}
        \begin{equation}
            ...
            \label{eq:omegaDef}
        \end{equation}
    We stress again that the indices in \Arf{eq:omegaDef} and the calculations
    ...
    \Arf{eq:omegaIncr}.
        \begin{align}
            ...
            \label{eq:omegaIncr}
        \end{align}
    where we used the properties listed in \Arf{eq:hermiteGaussProp} (mainly
    the recurrence) and the expansion in \Arf{eq:omegaDef}. The incrementation
    ...
    \Arf{eq:coeffdiffdef} the coefficients differentiated with respect to the
...
\subsection{Overlap Integral\label{susec:overlapIntegral}}
    ...
    \cite{handbookmath}.}, the results are presented in \Arf{eq:stypeOverlap}
    below.
        \begin{align}
            \Braket{i|j} &= \infint
        ...
        \end{align}
    The power $d$ in \Arf{eq:stypeOverlap} comes from splitting the integral
    into the $d$ dimensions. We are also using the \txtit{multi-index
    notation}\footnote{The power $d$ also means that with the multi-index
    notation the entire expression in the paranthesis are to be calculated for
    each dimension in $p$ and then multiplied
    together.}(\Arf{sec:multiIndexNotation}) and expanding
        \begin{equation}
...
\subsection{Coulomb Potential Integral}
    ...
    a simpler integral over a so-called \txtit{Coulomb Potential distribution}
        \begin{equation}
    ...
    terms of the \txtit{Modified Bessel function of the first kind} by using
    ...
    \txtit{incomplete Gamma function} From \Arf{eq:centerDerGauss}, the
...
\subsection{Coulomb Interaction Integral}
    ...
    case) is
        \begin{equation}
            ...
            (\blds{A}-\blds{B})^2}.
        \end{equation}
    Inserting this into \Arf{eq:Cintr1} with \Arf{eq:upsilonDef1}
        \begin{equation}
            \upsilon \equiv \left\{
                \begin{matrix}
...
\subsection{Recurrence Relation\label{sub:445}}
    ...
    and the first derivative with respect to $A_x$ is
        ...
        \end{align}
    With this we define an auxiliary function given in \Arf{eq:auxiliary2d1}
        \begin{equation}
            \begin{aligned}
    ...
    (\Arf{eq:coulombInteractionSimpleFinal}) follows this exact recurrence
    relation, but with a different proportionality factor
    $\alpha\beta/(\alpha+\beta)$. We will write it out in
    \Arf{eq:recurrInteraction} for the sake of clarity.
        \begin{equation}
            \begin{aligned}
...
\section{Double-Well Functions\label{sec:dwfunc}}
    ...
    assumed to be a polynomial of \txtit{even} degree $n$. This means that the
    integral over such a potential can be calculated using
    \Arf{eq:potIntegral}. Notice also that the $n$ degree polynomial only needs
    to be symmetric meaning $\abs{x}$ is also a valid polynomial to integrate
    over with \Arf{eq:potIntegral}.
...
\subsection{The Eigenvalue problem}
    ...
    Hartree-Fock are written in \Arf{eq:fullfinalsumary}.
        \begin{equation}
            \begin{aligned}
...
\subsection{Choosing the Basis Functions}
    ...
    functions to reach to \txtit{Hartree-Fock limit}. The choice will also
    follow the harmonic oscillator levels in terms of degeneracy(see
    \Arf{subfig:3Dspin} and \Arf{subfig:2Dspin}). This means for instance that
    ...
    This means that we choose the basis functions according to the \txtit{magic
    numbers} of the harmonic oscillator system basis.

    For the actual Hartree-Fock calculations it is worthwhile to mentioned that
    the expansion sum in \Arf{eq:dwexpansiondef} has to be truncated. The hope
    is that as we add more and more HO-functions to the eigenvalue problem
    (with the magic-number in mind), \Arf{eq:Heigdef}, the eignvalues will
    ...
    \Arf{tab:eig2D}. See also \Arf{sec:dwbimp} for details on the specific
    implementation.
...
\subsection{Degeneracy}
    ...
    respected in the calculations. More details are given in \Arf{chapter:5}.
...
\section{Summary\label{sec:basis_summary}}
    ...
    expansion coefficients, see \Arf{eq:overlapintdefsum} below.
        \begin{equation}
            \begin{aligned}
                \Omega_{ij}(\alpha,\beta,\blds{r},\blds{A},\blds{B}) &=
