...
\chapter{Many-Body Quantum Theory\label{chapter:3}}     
    ...
    also given while structure of program is given in \Arf{chapter:6}.
...
\section{Quantum Monte Carlo\label{sec:QMC}}
    ...
    \subsection{The Trial Wavefunction: Jastrow Factor\label{susec:TWFJastrow}}
        ...
        \end{itemize}
        In this thesis we explore two form, the popular \txtit{Pad\'e Function}
        and a more resent one build with a specific neural network known as a
        \txtit{Boltzmann Machine} >> REF THIS <<.
    
    \subsubsection{The Trial Wavefunction: Pad\'e
        Function\label{sususec:TWFPadeJastrow}} A popular form is the
        \txtit{Pad\'e-Jastrow} function. It is defined as\cite{basicMB,
        MCAbInitoChem}
            \begin{equation}
        ...
        so-called \txtit{cusp condition} which cancels the divergence of the
        Slater for particle positions close to one another.  Following
        \cite{jastcorratms, larsEivindThesis} the conditions are
            \begin{equation}
    ...
    \subsubsection{The Trial Wavefunction: Simple Exponential}
        >> REF THIS << With the mentioned features in mind, one of the simplest
        forms for a correlation factor is a scaled exponential evaluated with
        the inter-particle distance, this is\footnote{The $a$'s can also be
        ...
        section which with this function since it does give the mentions cusp
        conditions, something the soon-to-be presented NQS-wavefunction does
        not have.
    ...
    Wavefunction\label{sususec:NQSJastrow}}
        ...
        \txtit{neural network}. Popular >> ref something << approach is the
        \txtit{Restricted Boltzmann Machine} The form is
            \begin{equation}
                ...
                \label{eq:nqsdef}
            \end{equation}
        The derivatives and such is given in \Arf{appendix:B}.

    \subsection{The Trial Wavefunction: Connect the Jastrows}
        ...
        reason lies in the derivation of the conditions >> REF (NOT REALLY)
        derivate this maybe?) <<
